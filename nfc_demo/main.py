...
tf.app.flags.DEFINE_integer('test_neg', 1999, 'number of negative samples for test.')
...
class DataSet(object):
    ...
    @classmethod
    def read_data_set(cls, path, batch_size, user_negative, is_training, num_neg=4):
        data = pd.read_csv(path)
        if is_training:
            labels = np.ones(len(data), dtype=np.int32)
        else:
            labels = data['item_id'].tolist()
        features, labels = cls._add_negative(data, user_negative, labels, num_neg, is_training)
...
def main():
    user_negative = DataSet.read_user_bought("data/user_negative_train.csv")
    train_data = DataSet.read_data_set("data/train_ratings.csv", FLAGS.batch_size, user_negative, True,
                                       FLAGS.negative_num)
    test_data = DataSet.read_data_set("data/test_ratings.csv", FLAGS.test_neg+1, user_negative, False, FLAGS.test_neg)
