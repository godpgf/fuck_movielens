...
class WideDeepModel(object):
    def __init__(self, embed_size, user_size, item_size, dnn_hidden_units, lr,
    ...
    def get_data(self):
        ...
        self.gender = tf.one_hot(sample['gender'], 2)
        self.age = tf.one_hot(sample['age'], 7)
        self.occupation = tf.one_hot(sample['occupation'], 21)
        self.genres = tf.cast(sample['genres'], tf.float32)
    ...
    def create_model(self):
        ...
        # 得到深度模型
        # 深度用户隐藏向量
        w_user_deep = tf.Variable(tf.truncated_normal(shape=[self.user_size, self.embed_size], stddev=0.02))
        # 深度物品隐藏向量
        w_item_deep = tf.Variable(tf.truncated_normal(shape=[self.item_size, self.embed_size], stddev=0.02))
        # 通过用户的id取出对应的用户隐藏向量
        embd_user_deep = tf.nn.embedding_lookup(w_user_deep, self.user)
        # 通过物品id取出对应的隐藏向量
        embd_item_deep = tf.nn.embedding_lookup(w_item_deep, self.item)

        x = tf.concat([
            embd_user_deep, embd_item_deep,
            self.gender, self.age, self.occupation,
            self.genres
                       ], 1)
        input_size = self.embed_size + self.embed_size + 2 + 7 + 21 + 19
        reg_deep = tf.add(tf.nn.l2_loss(embd_user_deep), tf.nn.l2_loss(embd_item_deep))
        for _, hidden_unit in enumerate(self.dnn_hidden_units):
            w = tf.Variable(tf.truncated_normal(shape=[input_size, hidden_unit], stddev=0.02))
            b = tf.Variable(tf.constant(0.0, shape=[hidden_unit]))
            x = self.activation_func(tf.matmul(x, w) + b)
            input_size = hidden_unit
            reg_deep = tf.add(reg_deep, tf.nn.l2_loss(w))
        deep = tf.reduce_sum(x, 1)

        self.logits = wide + deep + bias_global
        ...
        with tf.name_scope("optimzation"):
            cost = self.loss + self.regularizer * reg_wide + self.regularizer * reg_deep
    ...
    def summary(self):
        ...
        self.writer = tf.summary.FileWriter('./graphs/WideDeep', tf.get_default_graph())
